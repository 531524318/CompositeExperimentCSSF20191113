package com.huazhi.changsha.compositeexperiment.activity.gas;
...
public class GasMainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                switch (msg.what) {
                    ...
                    case 1:
                        ...
                        if (command.substring(9, 13).equals("open")) {
                            start.setText("关闭监测");
                            ToastUtils.showToast(GasMainActivity.this, "监测已开启");
                        } else {
                            start.setText("开始监测");
                            ToastUtils.showToast(GasMainActivity.this, "监测已关闭");
                        }
                        break;
                }
            }
        ...
        Boolean isConn = (Boolean) Util.getSP(GasMainActivity.this, GlobalDefs.LS_SHARED_PREFS_NAME, GlobalDefs.KEY_IS_CONN_SOCKET, Boolean.class);
        if (isConn) {
            //获取当前报警功能状态
            LogUtils.i("获取当前报警功能状态");
            String order = "Hs_get_alarm_status_03T";
            //发送数据到a53
    ...
    private void initView() {
        tvTitle.setText(context.getString(R.string.composite_experiment_gas));
    ...
    public void clickStartBtn(View view) {
        ...
        if (isConn) {
            if (start.getText().equals("开始监测")) {
                String order = "Hs_start_alarm_fun_03T";
                //发送数据到a53
                EventBus.getDefault().post(new EventMsgModel(GlobalDefs.KEY_EVEN_BUS_MODULE_SEND_MSG_TO_A53, order));

            } else {
                String order = "Hs_close_alarm_fun_03T";
                //发送数据到a53
                EventBus.getDefault().post(new EventMsgModel(GlobalDefs.KEY_EVEN_BUS_MODULE_SEND_MSG_TO_A53, order));
            }
        } else {
    ...
    public void onEventMainThread(EventMsgModel event) {
        if (event.getMsg().endsWith(GlobalDefs.KEY_EVEN_BUS_MODULE_RECIVE_DATA)) {
            ...
            LogUtils.i("家庭可燃气体泄漏报警系统_接收到的数据=" + reciveData);
